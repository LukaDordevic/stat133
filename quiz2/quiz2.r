...
waiting_times <- function(inter, serv){
  ...
  for(i in 2:length(inter)){
    wait=c(wait, serv[i-1] + wait[i-1] - inter[i])
  }
  return(wait)
}
...
simulate500 <- function(k){
  for(i in 1:5){
    x=list( x, queueing_sim(k) )
  }
  return(x)
...
avg.wait = sim[[1$wait]]
avg.total = sim[[1$total]]
function(i in 2:500){
  avg.wait = c( avg.wait, mean(sim500[[i$wait]]))
  avg.total = c( avg.total, mean(sim500[[i$total]]))
  
}
avg_wait_total= matrix(c(avg.wait, avg.total), byrow=T, nrow=2)
